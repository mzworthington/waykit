...
Product bets and flags: [SOPs/hypothesis-driven-development.md](../../SOPs/hypothesis-driven-development.md). Confirmed PostHog rows: [product-signal-intake](../../SOPs/product-signal-intake.md) after restore `wk mcp default`. Confirmed crime-scene rows: [complexity-hotspots](../../SOPs/complexity-hotspots.md) §8. CI / telemetry hygiene: [quality-loops](../../SOPs/quality-loops.md) (group, rewrite, dedupe; do not open a PR). Start from operator-confirmed findings, deduplicate the board, and send bets without a card to [agent-prd](../agent-prd/SKILL.md) first. If Linear tools are missing after restore, draft INVEST text in the handover and stop — do not invent issue URLs.
