...
| CI / telemetry quality loops (failed check, Lighthouse, RUM, scout → Linear) | [quality-loops](../../../SOPs/quality-loops.md): one Automation per source files a typed ticket; Lighthouse drop vs last main → Performance (no chase 100, comment instead of cloning); hygiene; work picker reads `lists/work-picker-allowlist.yaml`. Cloud dashboard MCP missing → **BLOCKED**. Do not invent a loops skill. |
