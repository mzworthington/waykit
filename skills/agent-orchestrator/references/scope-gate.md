...
| Coding-agent tokens, USD cost, `gen_ai.*` / OTel traces | [coding-agent-observability](../../../SOPs/coding-agent-observability.md): host OTLP into community SigNoz Docker (`foundryctl`); query with `wk mcp signoz`. `wk eval` grades routing. PostHog stays product / RUM. Do not add an `agent-signoz` skill. Do not add OpenObserve. |
