...
| SonarQube / SonarCloud issues, hotspots, quality gate | [sonarqube-findings](../../../SOPs/sonarqube-findings.md): `wk mcp sonar`, triage into `handover_sonar.md`, skip kit disagreements (GitHub Actions stay `@vN`, never SHA). Apply `fix` rows in the parent. Do not add an `agent-sonarqube` skill. |
| CI / telemetry quality loops (failed check, Lighthouse, RUM, scout → Linear) | [quality-loops](../../../SOPs/quality-loops.md): one Automation per source files a typed ticket; hygiene; work picker reads `lists/work-picker-allowlist.yaml`. Cloud dashboard MCP missing → **BLOCKED**. Do not invent a loops skill. |
