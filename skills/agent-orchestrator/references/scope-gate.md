...
| Coding-agent tokens, USD cost, `gen_ai.*` / OTel traces | [coding-agent-observability](../../../SOPs/coding-agent-observability.md): `wk mcp signoz`, host OTLP into SigNoz. `wk eval` grades routing. PostHog stays product / RUM. Do not add an `agent-signoz` skill. Do not add OpenObserve. |
