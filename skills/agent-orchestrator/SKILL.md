...
| SonarQube issues ([sonarqube-findings](../../SOPs/sonarqube-findings.md)) | `wk mcp sonar`, triage, skip Actions SHA pins (`@vN`). Do not invent an `agent-sonarqube` skill. |
| CI / telemetry → Linear ([quality-loops](../../SOPs/quality-loops.md)) | File typed tickets (scout: one source, no PR), hygiene, then work picker. `agent-debug` for Bug. A Lighthouse drop vs last main files **Performance**; no ticket to chase 100; comment instead of cloning. Play: `agent-perf-opt`. A RUM / beacon break files a **Bug** on the owning repo (hostnames only). Dependabot / CodeQL stay on vendor PRs. No new specialist. Cloud dashboard catalog missing → **BLOCKED**. |
