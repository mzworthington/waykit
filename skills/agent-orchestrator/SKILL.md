...
| Coding-agent tokens / cost / traces ([coding-agent-observability](../../SOPs/coding-agent-observability.md)) | Community SigNoz Docker (`foundryctl`) on the operator machine; Cloud Agents use SigNoz Cloud ingest + hosted MCP. OTLP from Claude Code / Copilot / Cursor hooks. Query via `wk mcp signoz`. `wk eval` for right results. PostHog stays product. Do not invent an `agent-signoz` skill. |
