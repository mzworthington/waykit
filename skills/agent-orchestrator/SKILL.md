...
| CI / telemetry → Linear ([quality-loops](../../SOPs/quality-loops.md)) | File typed tickets (scout: one source, no PR). Dependabot / CodeQL stay on vendor PRs. Hygiene, then work picker. `agent-debug` for Bug. No new specialist. Cloud dashboard catalog missing → **BLOCKED**. |
