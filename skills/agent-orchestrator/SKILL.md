...
| CI / telemetry → Linear ([quality-loops](../../SOPs/quality-loops.md)) | File typed tickets (scout: one source, no PR), hygiene, then work picker. `agent-debug` for Bug. A Lighthouse drop vs last main files **Performance**; no ticket to chase 100; comment instead of cloning. Play: `agent-perf-opt`. Dependabot / CodeQL stay on vendor PRs. No new specialist. Cloud dashboard catalog missing → **BLOCKED**. |
