...
Coding-agent token usage, USD cost, `gen_ai.*` traces, Claude Code / Copilot / Cursor OTLP into SigNoz → `get_sop` `coding-agent-observability` (not PostHog, not a new `agent-signoz` skill). `wk eval` stays the quality gate.
