...
Quality loops, work picker, scheduled scout filing, playing an allowlisted ticket as a draft PR, or a Cloud Agent whose dashboard MCP is missing after `wk mcp --install` → `get_sop` `quality-loops` (not a new specialist). Stop **BLOCKED**; do not invent counts.
