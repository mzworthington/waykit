...
Quality loops, work picker, scheduled scout filing, Dependabot or CodeQL vendor PRs, or a Cloud Agent whose dashboard MCP is missing after `wk mcp --install` → `get_sop` `quality-loops` (not a new specialist). Stop **BLOCKED**; do not invent counts. Keep vendor PRs merge-ready; do not rewrite the lockfile.
