...
8. **Close the loop:** After the timebox, measure that leading indicator in PostHog (`wk mcp posthog --install`). Then confirm (default on, prune flag) or kill (flag off, prune slice) via `agent-user-stories` and `agent-prune`. Do not add a separate product-insights role. Turning live PostHog signals into backlog work is the two-session loop on [product signal intake](/SOPs/product-signal-intake) — link only; do not copy the procedure here. Git crime-scene ranking → Linear is [complexity hotspots](/SOPs/complexity-hotspots) §8 — same gate, no extra skill. SonarQube issues are [sonarqube findings](/SOPs/sonarqube-findings) — triage, skip kit disagreements, file one typed ticket after restore default, no extra skill. CI, RUM, Lighthouse, and scout alerts that should become typed Linear tickets are [quality loops](/SOPs/quality-loops) — allowlist plus draft PR, no extra skill.
