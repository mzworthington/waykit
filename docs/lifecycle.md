...
8. **Close the loop:** After the timebox, measure that leading indicator in PostHog (`wk mcp posthog --install`). Then confirm (default on, prune flag) or kill (flag off, prune slice) via `agent-user-stories` and `agent-prune`. Do not add a separate product-insights role. Turning live PostHog signals into backlog work is the two-session loop on [product signal intake](/SOPs/product-signal-intake) — link only; do not copy the procedure here. Git crime-scene ranking → Linear is [complexity hotspots](/SOPs/complexity-hotspots) §8 — same gate, no extra skill. SonarQube issues are [sonarqube findings](/SOPs/sonarqube-findings) — triage, skip kit disagreements, file one typed ticket after restore default, no extra skill. Coding-agent tokens, cost, and OTel traces are [coding-agent observability](/SOPs/coding-agent-observability) — community SigNoz Docker on the operator machine, SigNoz Cloud ingest for Cloud Agents, `wk mcp signoz` or hosted MCP, `wk eval` for right results, no extra skill. CI, RUM, Lighthouse, and scout alerts that should become typed Linear tickets are [quality loops](/docs/loops) — hygiene, allowlist, then draft PR, no extra skill.

[Orchestrator skill](https://github.com/mzworthington/waykit/blob/main/skills/agent-orchestrator/SKILL.md) · [Coding philosophy](https://github.com/mzworthington/waykit/blob/main/CODING_PHILOSOPHY.md) · [Quality loops](/docs/loops) · [EDD guide (alpha)](/docs/edd) · [Hosts](/docs/hosts) · [Subagent allowlist](/docs/subagents)
